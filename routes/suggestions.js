...
router.post('/pce', async (req, res) => {
    ...
    //console.log('suggestion', suggestion);
    const body = await suggestion.getAssociateBody() ;

    console.log('pce suggestion body', JSON.stringify(body));
    ...
    //parse response
    const {choices} = response.data;
    console.log('pce suggestion response', response.data);
    //save response to db
    await suggestion.update({choices});
    suggestion.save();
    //send response to client
